...
void User_Action::Load_Data() {
    std::fstream myFile(
        "UserData.txt",
        std::ios::out | std::ios::app);  // Tao file neu chua ton tai file san
    ...
    if (myFile.is_open()) {
        ...
        std::cout << "Trouble in loading user data" << std::endl;
}
...
void User_Action::Load_Money() {
    ...
    long long Money;
    if (myFile.is_open()) {
        while (myFile >> UserName >> Money) User_Money_Data[UserName] = Money;
        myFile.close();
    } else
        std::cout << "Trouble in loading user data" << std::endl;
}
...
void User_Action::User_Money(std::string UserName, long long Money) {
    ...
    if (myFile.is_open()) {
        ...
        std::cout << "Trouble in saving UserMoney!" << std::endl;
}
/* std::string User_Action::HashPassword(std::string password) {
        unsigned char hash[SHA256_DIGEST_LENGTH]; // Khoi tao mang Hash voi 256
bits SHA256((unsigned char*)password.c_str(), password.size(), hash); // Goi ham
ma hoa mat khau co san trong thu vien std::stringstream ss; for (int i = 0; i <
SHA256_DIGEST_LENGTH; i++) { ss << hex << setw(2) << setfill('0') <<
(int)hash[i];    // Chuyen qua he Hex
        }
        return ss.str();
}
*/
void User_Action::SignUp() {
    ...
    do {
        ...
        if (UserName.empty()) {
            std::cout << "Error: Username cannot be empty. Please try again."
                      << std::endl;
            continue;
        }
        ...
        if (UserName.length() < 3 || UserName.length() > 20) {
            std::cout << "Error: Username must be between 3 and 20 characters. "
                         "Please try again."
                      << std::endl;
            continue;
        }
        if (User_Data_Storage.find(UserName) != User_Data_Storage.end()) {
            std::cout << "Username is already available, please try again!"
                      << std::endl;  // Username da tung duoc luu
            continue;
        }
        ...
        std::cout
            ...
            << std::endl;  // Username chua tung duoc luu => khong trung ten
    ...
    while (true) {
        ...
        if (User_Data_Storage.find(UserName) != User_Data_Storage.end() &&
            ...
            std::cout << "Log in successfully"
                      << std::endl;  // Thong tin nhap trung khop voi thong tin
                                     // da duoc luu
            break;
        } else {
        TryAgain:
            ...
            char action;
            std::cin >> action;
            if (std::cin.fail() || action != 'y' || action != 'Y' ||
                action != 'n' || action != 'N') {
                std::cin.clear();
                std::cin.ignore(1000, '\n');
...
void User_Action::Update_Money(std::string UserName, long long MoneyChange) {
    if (User_Money_Data.find(UserName) != User_Money_Data.end()) {
        ...
        if (myFile.is_open()) {
            ...
            std::cout << "Trouble in updating money data!" << std::endl;
    } else
        std::cout << "User not found, cannot update money!" << std::endl;
}

// Hien thi so tien nguoi dung
void User_Action::Display_Money(std::string UserName) {
    if (User_Money_Data.find(UserName) != User_Money_Data.end())
        std::cout << "Current balance for " << UserName << ":"
                  << User_Money_Data[UserName] << "$" << std::endl;
    else
...
void User_Action::Display_Leader_Board(std::string Username) {
    ...
    for (auto x : Board) std::cout << x.first << " " << x.second << std::endl;
}
...
void User_Action::Load_Data_NumGame() {
    ...
    if (myFile.is_open()) {
        ...
        std::cout << "Can not open UserGamePlayed.txt file" << std::endl;
}
